...
router.post("/", auth_middleware_1.authenticateToken, async (req, res) => {
    ...
    try {
        console.log(`[${new Date().toISOString()}] Starting bon de commande operation`);
        const { description, employee_id, category_id, article_id, quantite_a_stocker, quantite_a_demander, article_name, target_date, } = req.body;
        ...
        if (!finalEmployeeId) {
            ...
            if (currentUserEmployee) {
                ...
                console.log("Created new employee record for current user:", finalEmployeeId);
            }
        }
        ...
        if (existingBonDeCommandeBasic) {
            ...
            if (existingArticleEntry) {
                ...
                const quantitiesChanged = existingArticleEntry.quantite_a_stocker !== quantite_a_stocker ||
                ...
                if (quantitiesChanged) {
                    ...
                    console.log("Quantities unchanged, no update needed for article:", article_id);
                }
            }
            else {
        ...
        const dateDescription = `Bon de commande du ${targetDate.toLocaleDateString("fr-FR", {
            year: "numeric",
            month: "long",
            day: "numeric",
        })}`;
        ...
        if (error.code === "P2002") {
            return res.status(400).json({
                error: "A bon de commande with this code already exists. Please try again.",
            });
        }
        if (error.code === "P2003") {
            return res.status(400).json({
                error: "Invalid reference. Please check that the category and article exist.",
            });
        }
        if (error.message) {
...
router.put("/category/:id", auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        console.log("[BonDeCommande API] Category updated successfully:", updatedCategory);
...
router.put("/:id/status", auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        console.log("[BonDeCommande API] Status updated successfully:", updatedBonDeCommande);
