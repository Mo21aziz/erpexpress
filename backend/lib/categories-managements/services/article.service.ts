  ...
  async createArticle(data: CreateArticleInput): Promise<Article> {
    ...
    if (data.numero !== undefined && data.numero !== null) {
      ...
      const created = await this.prisma.$transaction(async (tx) => {
        ...
        if (stillOccupied) {
          throw new Error(
            `Numero ${newNumero} is not available after shifting. Please try a different numero.`
          );
        }
        ...
        const created = await this.articleRepository.findByCondition(
          { where: { name: data.name, category_id: data.category_id } },
    ...
    if (data.numero !== undefined && data.numero !== null) {
      ...
      const updated = await this.prisma.$transaction(async (tx) => {
        ...
        const result = await tx.article.update({
          where: { id },
          data: { ...data, numero: newNumero } as any,
